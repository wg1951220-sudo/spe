...
export async function fetchDashboardData(persona: Persona, language: Language): Promise<DashboardData> {
  ...
  try {
    const fetchWithRetry = async (retries = 3, delay = 2000): Promise<any> => {
      try {
        ...
        const isServerError = error?.message?.includes("500") || error?.status === "INTERNAL" || error?.message?.includes("xhr error");
        
        if ((isQuotaError || isServerError) && retries > 0) {
          console.log(`API error (${error?.status || 'UNKNOWN'}), retrying in ${delay}ms... (${retries} retries left)`);
    ...
    const sanitizedData: DashboardData = {
      todaySignal: data.todaySignal || { title: "AI Pulse", description: "Intelligence feed", takeaway: "Stay tuned", url: "#" },
      metrics: Array.isArray(data.metrics) ? data.metrics : [],
      news: Array.isArray(data.news) ? data.news : [],
      socialSignals: Array.isArray(data.socialSignals) ? data.socialSignals : [],
    ...
    if (isQuotaError) {
      ...
      console.error("Error fetching dashboard data, falling back to static content:", error);
    }
